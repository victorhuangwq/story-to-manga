...
import { characterGenLogger, logApiRequest, logApiResponse, logError } from '@/lib/logger';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!characters || !setting || !style) {
      ...
      logApiResponse(characterGenLogger, endpoint, false, Date.now() - startTime, { error: 'Missing parameters' });
    ...
    for (const character of characters) {
      ...
      characterGenLogger.debug({ character_name: character.name }, 'Generating character reference');
...
`;
      ...
      try {
        ...
        logApiResponse(characterGenLogger, endpoint, false, Date.now() - startTime, { 
      ...
      if (characters.indexOf(character) < characters.length - 1) {
        characterGenLogger.debug({ delay_ms: 6500 }, 'Applying rate limit delay');
    ...
    logError(characterGenLogger, error, 'character reference generation');
    logApiResponse(characterGenLogger, endpoint, false, Date.now() - startTime, { error: 'Unexpected error' });
    return NextResponse.json(
