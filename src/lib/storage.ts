import type { CharacterReference, GeneratedPanel, StoryAnalysis, StoryBreakdown, ComicStyle } from '@/types';
...
export async function saveState(
  ...
  try {
    ...
    for (const panel of generatedPanels) {
      if (panel.image) {
        await imageStorage.storeImage(`panel-${panel.panelNumber}`, panel.image);
      }
    }
...
export async function loadState(): Promise<{
  ...
  try {
    ...
    console.log(`✅ State loaded successfully (${characterReferences.length} characters, ${generatedPanels.length} panels)`);
